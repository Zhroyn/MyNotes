...
若使用 2 的补码表示法，则无符号数和有符号数的加减法可以统一处理，只是理解不同罢了，例如 `3'b111 + 3'b111 = 3'b110` 既可以理解为得到 -2，也可以理解为得到 6。这是因为在 2 的补码表示法下，有符号数和无符号数一样都是连续的，只是溢出边界的位置不同。
...
- 进位选择加法器 (Carry Select Adder, CSA): 将全加器分组，每组先分别假设进位为 0 和 1 计算出两个结果，然后根据 $C_{in}$ 选择正确的结果，这样可以减少延迟，但是需要额外的硬件
...
- 向上舍入
- 向下舍入
- 向偶数舍入，为默认情况，当有两个最接近的值时，选择尾数为偶数的值
...
若对齐时右移一位，保留位可以保留住被移除的位，从而避免精度损失；若对齐右移大于一位，那么此时未移位的有效数字在 [1, 2) 范围内，被移位的有效数字在 [0, 1/2) 范围内，两数相减得到的数在 [1/2, 2) 范围内，因此规则化时最多左移一位。
